...
class clsControlesHTML
{
	...
	# Método para gerar grid de produtos
	public function geraGridProdutos($tabela)
	{
		$htmlTabela = '<table class="w3-table-all">
						<thead>
							<tr class="w3-red">
		...
		while ($linha = mysqli_fetch_row($tabela))
		{
			$select = $this->geraSelect('slcCategoria_'.$linha[0], $categoria->pegaCategorias(), $linha[3]);
			
			$htmlTabela .= '<tr>
								<td><img style="height:150px" src="imagens/'.$linha[1].'.jpg"></img><br><input type="file" size="30" name="txtFoto_'.$linha[0].'"/></td>
								<td><input type="text" size="20" name="txtNome_'.$linha[0].'" value="'.$linha[1].'"/></td>
